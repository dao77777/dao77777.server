...
class HomeService extends Service {
  async getArticleCardArray(pageNum) {
    ...
    const res = await mysql.query('select a.id, a.title, a.introduce, a.clickCount, a.readCount, a.likeCount, a.timeCreate, d.categoryName, e.tagName from (select a.*, count(b.id) as likeCount from (select a.*, count(b.id) as readCount from (select * from article limit ?,?) as a left join article_visitor_read as b on a.id=b.article_id group by a.id) as a left join article_visitor_like as b on a.id=b.article_id group by a.id) as a left join article_category as b on a.id=b.article_id left join article_tag as c on a.id=c.article_id left join category as d on b.category_id=d.id left join tag as e on c.tag_id=e.id;', [ begin, count ]);
  ...
  async routerLink_blogLink_getAuthorInfo() {
    ...
    const res = await mysql.query('select nickname, signature from blog');
    return { status: 200, statusMessage: '请求成功', data: res };
  }
